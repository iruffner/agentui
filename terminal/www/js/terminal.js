...
$hx_exports.terminal = $hx_exports.terminal || {};
$hx_exports.terminal.widget = $hx_exports.terminal.widget || {};
...
var terminal = {};
terminal.model = {};
terminal.model.EM = function() { };
$hxClasses["terminal.model.EM"] = terminal.model.EM;
terminal.model.EM.__name__ = ["terminal","model","EM"];
terminal.model.EM.addListener = function(id,func,listenerName) {
	return terminal.model.EM.delegate.addListener(id,func,listenerName);
};
terminal.model.EM.listenOnce = function(id,func,listenerName) {
	return terminal.model.EM.delegate.listenOnce(id,func,listenerName);
};
terminal.model.EM.removeListener = function(id,listenerUid) {
	terminal.model.EM.delegate.removeListener(id,listenerUid);
};
terminal.model.EM.change = function(id,t) {
	terminal.model.EM.delegate.change(id,t);
};
qoid.QE = function() { };
$hxClasses["qoid.QE"] = qoid.QE;
qoid.QE.__name__ = ["qoid","QE"];
terminal.widget = {};
terminal.widget.DialogManager = $hx_exports.terminal.widget.DialogManager = function() { };
$hxClasses["terminal.widget.DialogManager"] = terminal.widget.DialogManager;
terminal.widget.DialogManager.__name__ = ["terminal","widget","DialogManager"];
terminal.widget.DialogManager.showDialog = function(dialogFcnName,options) {
...
terminal.widget.DialogManager.showLogin = function() {
	terminal.widget.DialogManager.showDialog("loginDialog");
};
terminal.widget.DialogManager.showCreateAgent = function() {
	terminal.widget.DialogManager.showDialog("createAgentDialog");
};
...
qoid.ResponseProcessor.processResponse = function(dataArr) {
	Lambda.iter(dataArr,function(data) {
		if(!data.success) m3.log.Logga.get_DEFAULT().error(Std.string(data.error.message) + " | " + Std.string(data.error.stacktrace)); else {
...
terminal.Terminal = $hx_exports.terminal.Terminal = function() { };
$hxClasses["terminal.Terminal"] = terminal.Terminal;
terminal.Terminal.__name__ = ["terminal","Terminal"];
terminal.Terminal.main = function() {
	terminal.api.EventDelegate.init();
	terminal.TerminalContext.init();
};
terminal.Terminal.start = function() {
	new $("body").click(function() {
		new $(".nonmodalPopup").hide();
	});
	var content = new $(".content");
	var contentAuditLog = new $("<div></div>");
	contentAuditLog.appendTo(content);
	contentAuditLog.auditLog();
	haxe.Timer.delay(function() {
		terminal.widget.DialogManager.showLogin();
	},100);
};
terminal.TerminalContext = function() { };
$hxClasses["terminal.TerminalContext"] = terminal.TerminalContext;
terminal.TerminalContext.__name__ = ["terminal","TerminalContext"];
terminal.TerminalContext.init = function() {
	terminal.TerminalContext.registerListeners();
	terminal.TerminalContext.AUDIT_LOGS = new m3.observable.ObservableSet(qoid.model.ModelObjWithIid.identifier);
};
terminal.TerminalContext.registerListeners = function() {
	terminal.model.EM.listenOnce(qoid.QE.onInitialDataload,terminal.TerminalContext._onInitialDataLoadComplete,"TerminalContext-onInitialDataLoad");
	terminal.model.EM.addListener("onAuditLog",terminal.TerminalContext._onAuditLog,"TerminalContext-onAuditLog");
};
terminal.TerminalContext._onInitialDataLoadComplete = function(n) {
	terminal.model.EM.change("QueryAuditLogs");
};
terminal.TerminalContext._onAuditLog = function(data) {
	if(m3.helper.ArrayHelper.hasValues(data.result.results)) {
		...
		while(_g < _g1.length) {
			var result = _g1[_g];
			++_g;
			var c = m3.serialization.Serializer.get_instance().fromJsonX(result,terminal.model.AuditLogContent);
			if(c != null) terminal.TerminalContext.AUDIT_LOGS.addOrUpdate(c);
		}
	}
};
terminal.api = {};
terminal.api.EventDelegate = function() { };
$hxClasses["terminal.api.EventDelegate"] = terminal.api.EventDelegate;
terminal.api.EventDelegate.__name__ = ["terminal","api","EventDelegate"];
terminal.api.EventDelegate.init = function() {
	terminal.model.EM.addListener("QueryAuditLogs",function(c) {
		var query = "hasLabelPath('Meta','Audit Log') and contentType = '" + terminal.model.TerminalContentTypes.AUDIT_LOG + "'";
		qoid.QoidAPI.query(new qoid.RequestContext("AuditLog"),"content",query,false,true);
	});
	terminal.model.EM.addListener("CreateAgent",function(user) {
		qoid.QoidAPI.createAgent(user.name,user.pwd);
	});
	terminal.model.EM.addListener("UserLogout",function(c1) {
		qoid.QoidAPI.logout();
	});
};
terminal.model.EMEvent = function() { };
$hxClasses["terminal.model.EMEvent"] = terminal.model.EMEvent;
terminal.model.EMEvent.__name__ = ["terminal","model","EMEvent"];
terminal.model.TerminalContentTypes = function() { };
$hxClasses["terminal.model.TerminalContentTypes"] = terminal.model.TerminalContentTypes;
terminal.model.TerminalContentTypes.__name__ = ["terminal","model","TerminalContentTypes"];
terminal.model.AuditLogContentData = function() {
	qoid.model.ContentData.call(this);
};
$hxClasses["terminal.model.AuditLogContentData"] = terminal.model.AuditLogContentData;
terminal.model.AuditLogContentData.__name__ = ["terminal","model","AuditLogContentData"];
terminal.model.AuditLogContentData.__super__ = qoid.model.ContentData;
terminal.model.AuditLogContentData.prototype = $extend(qoid.model.ContentData.prototype,{
	__class__: terminal.model.AuditLogContentData
});
terminal.model.AuditLogContent = function() {
	qoid.model.Content.call(this,terminal.model.TerminalContentTypes.AUDIT_LOG,terminal.model.AuditLogContentData);
};
$hxClasses["terminal.model.AuditLogContent"] = terminal.model.AuditLogContent;
terminal.model.AuditLogContent.__name__ = ["terminal","model","AuditLogContent"];
terminal.model.AuditLogContent.__super__ = qoid.model.Content;
terminal.model.AuditLogContent.prototype = $extend(qoid.model.Content.prototype,{
	__class__: terminal.model.AuditLogContent
});
terminal.model.TerminalContentHandler = function() {
};
$hxClasses["terminal.model.TerminalContentHandler"] = terminal.model.TerminalContentHandler;
terminal.model.TerminalContentHandler.__name__ = ["terminal","model","TerminalContentHandler"];
terminal.model.TerminalContentHandler.__interfaces__ = [m3.serialization.TypeHandler];
terminal.model.TerminalContentHandler.prototype = {
	read: function(fromJson,reader,instance) {
		var obj = null;
		try {
			var _g = fromJson.contentType;
			switch(_g) {
			case terminal.model.TerminalContentTypes.AUDIT_LOG:
				obj = m3.serialization.Serializer.get_instance().fromJsonX(fromJson,terminal.model.AuditLogContent);
				break;
			}
		} catch( err ) {
		}
	...
	,__class__: terminal.model.TerminalContentHandler
};
...
terminal.model.EM.delegate = m3.event.EventManager.get_instance();
var defineWidget = function() {
	return { _create : function() {
		...
		terminal.model.EM.change("CreateAgent",newUser);
		terminal.model.EM.listenOnce(qoid.QE.onAgentCreated,function(n) {
...
var defineWidget = function() {
	return { _create : function() {
		...
		m3.jq.PlaceHolderUtil.setFocusBehavior(self.input_pw,self.placeholder_pw);
		terminal.model.EM.addListener(qoid.QE.onInitialDataload,function(n) {
		...
		var dlgOptions = { autoOpen : false, title : "Login", height : 280, width : 400, modal : true, buttons : { Login : function() {
			...
			terminal.widget.DialogManager.showCreateAgent();
...
var defineWidget = function() {
	return { _create : function() {
		var self = this;
		var selfElement = this.element;
		if(!selfElement["is"]("div")) throw new m3.exception.Exception("Root of AuditLogEntry must be a div element");
		selfElement.addClass("_auditLogEntry");
		var $it0 = qoid.Qoid.connections.iterator();
		while( $it0.hasNext() ) {
			var connection = $it0.next();
			m3.log.Logga.get_DEFAULT().debug("CONNECTION: " + connection.data.name);
		}
		var $it1 = qoid.Qoid.profiles.iterator();
		while( $it1.hasNext() ) {
			var profile = $it1.next();
			m3.log.Logga.get_DEFAULT().debug("PROFILE: " + profile.name);
		}
		var $it2 = qoid.Qoid.aliases.iterator();
		while( $it2.hasNext() ) {
			var alias = $it2.next();
			m3.log.Logga.get_DEFAULT().debug("ALIAS: " + alias.profile.name);
		}
		m3.log.Logga.get_DEFAULT().debug("CURRENT ALIAS: " + qoid.Qoid.get_currentAlias().profile.name);
		var divProfile = new $("<div class='profile'></div>").appendTo(selfElement);
		var divProfileImage = new $("<div class='profileImage'></div>").appendTo(divProfile).append(self.getProfileImageHtml());
		var divProfileName = new $("<div class='profileName'></div>").appendTo(divProfile).append(self.getProfileNameHtml());
		var divDetails = new $("<div class='details'></div>").appendTo(selfElement);
		var divKind = new $("<div class='kind'></div>").appendTo(divDetails).append(self.getKindHtml());
		var divSuccessful = new $("<div class='successful'></div>").appendTo(divDetails).append(self.getSuccessfulHtml());
		var divErrorCode = new $("<div class='errorCode'></div>").appendTo(divDetails).append(self.getErrorCodeHtml());
		...
		var self1 = this;
		var connectionIid = self1.options.auditLogContent.props.route[0];
		if(qoid.Qoid.get_currentAlias().connectionIid == connectionIid) return ((function($this) {
			var $r;
		...
		var self2 = this;
		var profile1 = self2.getProfile();
		if(profile1.imgSrc == null || profile1.imgSrc == "") return "<img src='media/default_avatar.jpg' />"; else return "<img src='" + self2.getProfile().imgSrc + "' />";
	}, getProfileNameHtml : function() {
		var self3 = this;
		return self3.getProfile().name;
	}, getKindHtml : function() {
		var self4 = this;
		var _g = self4.options.auditLogContent.props.kind;
		switch(_g) {
		case "QueryRequest":
			return "Query";
		case "CancelQueryRequest":
			return "Cancel Query";
		case "CreateAliasRequest":
			return "Create Alias";
		case "UpdateAliasRequest":
			return "Update Alias";
		case "DeleteAliasRequest":
			return "Delete Alias";
		case "CreateAliasLoginRequest":
			return "Create Alias Login";
		case "UpdateAliasLoginRequest":
			return "Update Alias Login";
		case "DeleteAliasLoginRequest":
			return "Delete Alias Login";
		case "UpdateAliasProfileRequest":
			return "Update Alias Profile";
		...
		case "AddContentLabelRequest":
			return "Add Content Label";
		case "RemoveContentLabelRequest":
			return "Remove Content Label";
		case "CreateLabelRequest":
			return "Create Label";
		case "UpdateLabelRequest":
			return "Update Label";
		case "MoveLabelRequest":
			return "Move Label";
		case "CopyLabelRequest":
			return "Copy Label";
		case "RemoveLabelRequest":
			return "Remove Label";
		case "GrantLabelAccessRequest":
			return "Grant Label Access";
		case "RevokeLabelAccessRequest":
			return "Revoke Label Access";
		case "UpdateLabelAccessRequest":
			return "Update Label Access";
		case "CreateNotificationRequest":
			return "Create Notification";
		case "ConsumeNotificationRequest":
			return "Consume Notification";
		case "DeleteNotificationRequest":
			return "Delete Notification";
		case "InitiateIntroductionRequest":
			return "Initiate Introduction";
		case "IntroductionRequest":
			return "Introduction";
		case "AcceptIntroductionRequest":
			return "Accept Introduction";
		default:
			return self4.options.auditLogContent.props.kind;
		}
	}, getSuccessfulHtml : function() {
		var self5 = this;
		return "Successful: " + (self5.options.auditLogContent.props.success?"True":"False");
	}, getErrorCodeHtml : function() {
		var self6 = this;
		return "Error Code: " + (self6.options.auditLogContent.props.errorCode == null?"None":self6.options.auditLogContent.props.errorCode);
	}, getDegreesOfTrustHtml : function() {
		var self7 = this;
		return "Degrees of Trust: " + self7.options.auditLogContent.props.route.length;
	}, getTimestampHtml : function() {
		var self8 = this;
		return "Timestamp: " + HxOverrides.dateStr(self8.options.auditLogContent.created);
	}, getDataHtml : function() {
		var self9 = this;
		return JSON.stringify(self9.options.auditLogContent.props.messageData);
	}};
};
$.widget("ui.auditLogEntry",defineWidget());
var defineWidget = function() {
	return { _registerListeners : function() {
		var self = this;
		var selfElement = this.element;
		self._onAuditLog = function(content,evt) {
		...
		var self1 = this;
		var selfElement1 = this.element;
		if(!selfElement1["is"]("div")) throw new m3.exception.Exception("Root of AuditLog must be a div element");
		selfElement1.addClass("_auditLog");
		self1._registerListeners();
	}, destroy : function() {
		var self2 = this;
		terminal.TerminalContext.AUDIT_LOGS.removeListener(self2._onAuditLog);
		$.Widget.prototype.destroy.call(this);
	}};
};
$.widget("ui.auditLog",defineWidget());
...
qoid.model.VerificationResponse.__rtti = "<class path=\"qoid.model.VerificationResponse\" params=\"\" module=\"qoid.model.ModelObj\">\n\t<notificationIid public=\"1\"><c path=\"String\"/></notificationIid>\n\t<verificationContent public=\"1\"><c path=\"String\"/></verificationContent>\n\t<connectionIid public=\"1\"><c path=\"String\"/></connectionIid>\n\t<contentIid public=\"1\"><c path=\"String\"/></contentIid>\n\t<new public=\"1\" set=\"method\" line=\"618\"><f a=\"vr:verificationContent\">\n\t<c path=\"qoid.model.VerificationRequestNotification\"/>\n\t<c path=\"String\"/>\n\t<x path=\"Void\"/>\n</f></new>\n\t<meta><m n=\":rtti\"/></meta>\n</class>";
qoid.model.IntroResponseMessage.__rtti = "<class path=\"qoid.model.IntroResponseMessage\" params=\"\" module=\"qoid.model.ModelObj\">\n\t<notificationIid public=\"1\"><c path=\"String\"/></notificationIid>\n\t<accepted public=\"1\"><x path=\"Bool\"/></accepted>\n\t<new public=\"1\" set=\"method\" line=\"691\"><f a=\"notificationIid:accepted\">\n\t<c path=\"String\"/>\n\t<x path=\"Bool\"/>\n\t<x path=\"Void\"/>\n</f></new>\n\t<meta><m n=\":rtti\"/></meta>\n</class>";
terminal.model.EMEvent.QueryAuditLogs = "QueryAuditLogs";
terminal.model.EMEvent.OnAuditLog = "onAuditLog";
terminal.model.EMEvent.CreateAgent = "CreateAgent";
terminal.model.EMEvent.UserLogout = "UserLogout";
terminal.model.TerminalContentTypes.AUDIT_LOG = "AUDIT_LOG";
terminal.model.AuditLogContentData.__rtti = "<class path=\"terminal.model.AuditLogContentData\" params=\"\" module=\"terminal.model.TerminalModel\">\n\t<extends path=\"qoid.model.ContentData\"/>\n\t<kind public=\"1\"><c path=\"String\"/></kind>\n\t<route public=\"1\"><c path=\"Array\"><c path=\"String\"/></c></route>\n\t<messageData public=\"1\"><d/></messageData>\n\t<success public=\"1\"><x path=\"Bool\"/></success>\n\t<errorCode public=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta><m n=\":optional\"/></meta>\n\t</errorCode>\n\t<new public=\"1\" set=\"method\" line=\"18\"><f a=\"\"><x path=\"Void\"/></f></new>\n</class>";
terminal.model.AuditLogContent.__rtti = "<class path=\"terminal.model.AuditLogContent\" params=\"\" module=\"terminal.model.TerminalModel\">\n\t<extends path=\"qoid.model.Content\"><c path=\"terminal.model.AuditLogContentData\"/></extends>\n\t<new public=\"1\" set=\"method\" line=\"24\"><f a=\"\"><x path=\"Void\"/></f></new>\n</class>";
