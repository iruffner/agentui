...
qoid.QoidAPI.acceptVerification2 = function(context,data) {
	...
	var verification = new qoid.model.ContentVerification(vr.data.verifierId,vr.data.contentIid);
...
pagent.PinterAgent.main = function() {
	...
	pagent.model.EM.addListener(qoid.QE.onAliasLoaded,function(a) {
		...
		if(qoid.Qoid.get_currentAlias() != null && qoid.Qoid.get_currentAlias().iid == a.iid) window.document.title = a.profile.name + " | PinterQoid";
...
pagent.api.EventDelegate.init = function() {
	...
	pagent.model.EM.addListener("CreateContent",function(data) {
		qoid.QoidAPI.createContent(data.content.contentType,m3.serialization.Serializer.get_instance().toJson(data.content).data,data.labelIids,null,null,data.semanticId);
	});
