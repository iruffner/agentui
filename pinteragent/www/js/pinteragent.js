...
qoid.ResponseProcessor.processResponse = function(dataArr) {
	Lambda.iter(dataArr,function(data) {
		if(!data.success) {
			...
			} else if(context.context == "verificationContent") qoid.ResponseProcessor.updateModelObject(result.type,result.action,result.results); else if(!qoid.Synchronizer.processResponse(context,data)) {
				if(result != null) {
