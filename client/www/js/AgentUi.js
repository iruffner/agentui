...
ui.api.BennuHandler.prototype = {
	...
	,deleteLabel: function(data) {
		...
		var labelledContent = new m3.observable.FilteredSet(ui.AppContext.LABELEDCONTENT,function(lc) {
			return lc.labelIid == data.label.get_iid();
		}).asArray();
		var labels = ui.AppContext.getLabelDescendents(data.label.get_iid()).asArray();
		...
		var _g = 0;
		while(_g < labelledContent.length) {
			var lc = labelledContent[_g];
			++_g;
			lc.deleted = true;
	...
	,createLabel: function(data) {
		var lc = new ui.model.LabelChild(data.parentIid,data.label.get_iid());
	...
	,updateContent: function(data) {
		...
		while( $it0.hasNext() ) {
			...
			while(_g < _g1.length) {
				var label = _g1[_g];
				++_g;
				if(label.get_iid() == labeledContent.labelIid) {
					found = true;
					break;
				}
			}
		...
		while(_g < _g1.length) {
			...
			if(!found) labelsToAdd.push(new ui.model.LabeledContent(data.content.get_iid(),label.get_iid()));
		}
...
ui.api.ChannelRequestMessageBundle = function(requests_) {
	this.agentId = ui.AppContext.AGENT.iid;
...
ui.api.MsgType.introductionConfirmationResponse = ["introductionConfirmationResponse",18];
...
ui.api.ResponseProcessor.updateModel = function(data,type) {
	...
	case "alias":
		...
		alias.deleted = true;
		ui.AppContext.AGENT.aliasSet.addOrUpdate(alias);
		break;
	case "content":
		var content = m3.helper.OSetHelper.getElement(ui.AppContext.MASTER_CONTENT,data.primaryKey);
		content.deleted = true;
		ui.AppContext.MASTER_CONTENT.addOrUpdate(content);
		break;
	case "label":
		var label = m3.helper.OSetHelper.getElement(ui.AppContext.MASTER_LABELS,data.primaryKey);
		label.deleted = true;
		ui.AppContext.MASTER_LABELS.addOrUpdate(label);
		break;
	case "labelChild":
		var lc = m3.helper.OSetHelper.getElement(ui.AppContext.MASTER_LABELCHILDREN,data.primaryKey);
		lc.deleted = true;
		ui.AppContext.MASTER_LABELCHILDREN.addOrUpdate(lc);
		break;
	case "labeledContent":
		var lc = m3.helper.OSetHelper.getElement(ui.AppContext.MASTER_LABELEDCONTENT,data.primaryKey);
		lc.deleted = true;
...
ui.model.EditLabelData = function(label,parentIid) {
	this.label = label;
	this.parentIid = parentIid;
};
...
var defineWidget = function() {
	return { options : { itemsClass : null}, _create : function() {
		...
		ui.model.EM.addListener(ui.model.EMEvent.TARGET_CHANGE,new ui.model.EMListener(function(conn) {
			if(conn != null) self._setConnections(conn.connectionSet); else ui.AppContext.LOGGER.warn("fix me self._setConnections(alias.connectionSet)");
...
var defineWidget = function() {
	return { _getDragStop : function() {
		...
		if(ui.AppContext.GROUPED_LABELEDCONTENT.delegate().get(self1.options.content.get_iid()) == null) ui.AppContext.GROUPED_LABELEDCONTENT.addEmptyGroup(self1.options.content.get_iid());
		...
		self1.mappedLabels = new m3.observable.MappedSet(ui.AppContext.GROUPED_LABELEDCONTENT.delegate().get(self1.options.content.get_iid()),function(lc) {
...
var defineWidget = function() {
	return { _createWidgets : function(selfElement,self) {
		...
		if(ui.AppContext.GROUPED_LABELEDCONTENT.delegate().get(self.options.content.get_iid()) == null) ui.AppContext.GROUPED_LABELEDCONTENT.addEmptyGroup(self.options.content.get_iid());
		...
		self.mappedLabels = new m3.observable.MappedSet(ui.AppContext.GROUPED_LABELEDCONTENT.delegate().get(self.options.content.get_iid()),function(lc) {
	...
	return { _showNewLabelPopup : function(reference,isUpdate) {
		...
		popup = popup.popup({ createFcn : function(el) {
			...
			updateLabel = function() {
				...
				ui.AppContext.LOGGER.info("Update label | " + label.get_iid());
				label.name = input.val();
...
ui.model.Content.__rtti = "<class path=\"ui.model.Content\" params=\"T\" module=\"ui.model.ModelObj\">\n\t<extends path=\"ui.model.ModelObjWithIid\"/>\n\t<contentType public=\"1\"><e path=\"ui.model.ContentType\"/></contentType>\n\t<data><d/></data>\n\t<props public=\"1\">\n\t\t<c path=\"ui.model.Content.T\"/>\n\t\t<meta><m n=\":transient\"/></meta>\n\t</props>\n\t<creator public=\"1\">\n\t\t<c path=\"String\"/>\n\t\t<meta><m n=\":optional\"/></meta>\n\t</creator>\n\t<created public=\"1\" get=\"accessor\" set=\"null\">\n\t\t<c path=\"Date\"/>\n\t\t<meta>\n\t\t\t<m n=\":transient\"/>\n\t\t\t<m n=\":isVar\"/>\n\t\t</meta>\n\t</created>\n\t<modified public=\"1\" get=\"accessor\" set=\"null\">\n\t\t<c path=\"Date\"/>\n\t\t<meta>\n\t\t\t<m n=\":transient\"/>\n\t\t\t<m n=\":isVar\"/>\n\t\t</meta>\n\t</modified>\n\t<type>\n\t\t<x path=\"Class\"><c path=\"ui.model.Content.T\"/></x>\n\t\t<meta><m n=\":transient\"/></meta>\n\t</type>\n\t<get_created public=\"1\" set=\"method\" line=\"361\"><f a=\"\"><c path=\"Date\"/></f></get_created>\n\t<get_modified public=\"1\" set=\"method\" line=\"365\"><f a=\"\"><c path=\"Date\"/></f></get_modified>\n\t<setData public=\"1\" set=\"method\" line=\"370\"><f a=\"data\">\n\t<d/>\n\t<x path=\"Void\"/>\n</f></setData>\n\t<readResolve set=\"method\" line=\"374\"><f a=\"\"><x path=\"Void\"/></f></readResolve>\n\t<writeResolve set=\"method\" line=\"378\"><f a=\"\"><x path=\"Void\"/></f></writeResolve>\n\t<getTimestamp public=\"1\" set=\"method\" line=\"382\"><f a=\"\"><c path=\"String\"/></f></getTimestamp>\n\t<objectType public=\"1\" set=\"method\" line=\"386\" override=\"1\"><f a=\"\"><c path=\"String\"/></f></objectType>\n\t<new public=\"1\" set=\"method\" line=\"353\"><f a=\"contentType:type\">\n\t<e path=\"ui.model.ContentType\"/>\n\t<x path=\"Class\"><c path=\"ui.model.Content.T\"/></x>\n\t<x path=\"Void\"/>\n</f></new>\n</class>";
