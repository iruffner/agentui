...
ui.AgentUi.start = function() {
	...
	ui.AgentUi.HOT_KEY_ACTIONS.push(function(evt) {
		if(evt.altKey && evt.shiftKey && evt.keyCode == 78) {
			...
			var connection = ui.AppContext.get_alias().get_connectionSet().asArray()[2];
...
ui.AppContext.get_alias = function() {
	return ui.AppContext.AGENT.get_currentAlias();
}
ui.AppContext.set_alias = function(alias) {
...
ui.api.BennuHandler.prototype = {
	...
	,deleteLabel: function(label) {
		...
		while(_g < labelChildren.length) {
			...
			++_g;
			requests.push(new ui.api.ChannelRequestMessage(ui.api.BennuHandler.DELETE,context + "labelChild",ui.api.DeleteMessage.create(labelChild)));
		}
	...
	,getAliasLabels: function(alias) {
		ui.AppContext.get_alias().get_labelSet().clear();
		ui.AppContext.get_alias().get_labelSet().addAll(ui.AppContext.getLabelChildren(alias.rootLabelIid).asArray());
	}
...
ui.api.DeleteMessage.create = function(object) {
	return new ui.api.DeleteMessage(object.objectType(),object.get_iid());
}
...
ui.api.ChannelRequestMessageBundle = function(requests_) {
	this.channel = ui.AppContext.CHANNEL;
...
ui.api.EventDelegate.prototype = {
	_setUpEventListeners: function() {
		...
		this.processHash.set(ui.api.MsgType.getAliasConnectionsResponse,function(data) {
			...
			ui.AppContext.get_alias().get_connectionSet().clear();
			ui.AppContext.get_alias().get_connectionSet().addAll(resp.contentImpl.connections);
		});
		...
		this.processHash.set(ui.api.MsgType.getAliasLabelsResponse,function(data) {
			...
			ui.AppContext.get_alias().get_labelSet().clear();
			ui.AppContext.get_alias().get_labelSet().addAll(resp.contentImpl.get_aliasLabels());
		});
...
ui.api.MsgType.crudMessage = ["crudMessage",54];
...
ui.api.LongPollingRequest.prototype = $extend(ui.api.BaseRequest.prototype,{
	poll: function() {
		if(this.running) {
			this.baseOpts.url = ui.AgentUi.URL + "/api/channel/poll?channel=" + ui.AppContext.CHANNEL + "&timeoutMillis=" + Std.string(this.timeout);
...
ui.api.ResponseProcessor.initialDataLoad = function(data) {
	...
	ui.AppContext.set_alias(ui.AppContext.AGENT.aliasSet.iterator().next());
...
ui.api.Synchronizer.prototype = {
	dataReceived: function(data,responseType) {
		...
		case "alias":
			var $it0 = ui.AppContext.AGENT.aliasSet.iterator();
			while( $it0.hasNext() ) {
				var alias = $it0.next();
				if(alias.get_iid() == data.primaryKey) this.parms.aliases.push(alias);
			}
			break;
		case "label":
			...
			break;
		case "labelChild":
			var $it1 = ui.AppContext.LABELCHILDREN.iterator();
			while( $it1.hasNext() ) {
				var lc = $it1.next();
				if(lc.get_iid() == data.primaryKey) this.parms.labelChildren.push(lc);
			}
			break;
...
ui.helper.ModelHelper.asConnection = function(alias) {
	...
	conn.profile = new ui.model.UserData(alias.name,(function($this) {
		var $r;
		try {
			$r = alias.data.imgSrc;
		} catch( __e ) {
...
ui.model.EMEvent.INTRODUCTION_CONFIRMATION_RESPONSE = ["INTRODUCTION_CONFIRMATION_RESPONSE",33];
...
ui.model.EMEvent.INTRODUCTION_NOTIFICATION = ["INTRODUCTION_NOTIFICATION",34];
...
ui.model.ModelObjWithIid = function() {
	...
	this.set_iid(m3.util.UidGenerator.create(32));
	this.deleted = false;
	this.agentId = "";
};
...
ui.model.ModelObjWithIid.prototype = $extend(ui.model.ModelObj.prototype,{
	...
	,get_iid: function() {
		if(m3.helper.StringHelper.isBlank(this.iid)) this.set_iid(m3.util.UidGenerator.create(32));
		return this.iid;
	}
...
ui.model.Agent = function() {
	...
	this.aliasSet = new m3.observable.ObservableSet(ui.model.Alias.identifier);
...
ui.model.Agent.prototype = $extend(ui.model.ModelObj.prototype,{
	getSelfConnection: function() {
		...
		conn.label = this.get_currentAlias().name;
		return conn;
	}
	,writeResolve: function() {
	...
	,get_currentAlias: function() {
		if(this.currentAlias == null && !this.aliasSet.isEmpty()) this.set_currentAlias(this.aliasSet.iterator().next()); else if(this.currentAlias == null) {
	...
	,registerModelListeners: function() {
		var _g2 = this;
		ui.model.EM.addListener(ui.model.EMEvent.DeleteLabel,new ui.model.EMListener(function(labels) {
			var _g1 = 1, _g = labels.length;
			while(_g1 < _g) {
				var i = _g1++;
				_g2.get_currentAlias().get_labelSet()["delete"](labels[i]);
			}
			ui.model.EM.change(ui.model.EMEvent.FitWindow);
		},"User-DeleteLabel"));
		ui.model.EM.addListener(ui.model.EMEvent.ConnectionUpdate,new ui.model.EMListener(function(conn) {
			_g2.get_currentAlias().get_connectionSet().update(conn);
			ui.model.EM.change(ui.model.EMEvent.FitWindow);
		},"User-ConnUpdate"));
...
ui.model.CreateLabelData = function(label,parentIid) {
	this.label = label;
	this.parentIid = parentIid;
};
$hxClasses["ui.model.CreateLabelData"] = ui.model.CreateLabelData;
ui.model.CreateLabelData.__name__ = ["ui","model","CreateLabelData"];
ui.model.CreateLabelData.prototype = {
	__class__: ui.model.CreateLabelData
}
...
ui.widget.score.ContentTimeLine.prototype = {
	...
	,addContentElement: function(content,ele) {
		...
		ele.attr({ id : content.creator + "-" + content.get_iid()});
	...
	return { _create : function() {
		...
		if(alias != null) {
			var loadAliasBtn = new $("<button class='fleft'>Use This Alias</button>").appendTo(leftDiv).button().click(function(evt) {
				ui.AppContext.set_alias(alias);
				ui.model.EM.change(ui.model.EMEvent.LOAD_ALIAS,alias);
			...
			if(m3.helper.StringHelper.isNotBlank((function($this) {
				var $r;
				try {
					$r = ui.AppContext.AGENT.userData.imgSrc;
				} catch( __e ) {
					$r = "";
				}
				return $r;
			}(this)))) imgSrc = ui.AppContext.AGENT.userData.imgSrc;
			leftDiv.append(new $("<img alt='alias' src='" + imgSrc + "' class='userImg shadow'/>"));
			leftDiv.append(new $("<h2>" + ui.AppContext.AGENT.userData.name + "</h2>"));
		}
		...
		var span = new $("<span class='clickable'></span>").appendTo(rightDiv).click(function(evt) {
			...
			var $r;
			try {
				$r = ui.AppContext.AGENT.userData.name;
			} catch( __e ) {
				$r = "";
			}
			return $r;
		...
		Lambda.iter(ui.AppContext.AGENT.aliasSet,function(a) {
		...
		Lambda.iter(ui.AppContext.AGENT.aliasSet,function(a1) {
...
var defineWidget = function() {
	return { options : { itemsClass : null}, _create : function() {
		...
		ui.model.EM.addListener(ui.model.EMEvent.TARGET_CHANGE,new ui.model.EMListener(function(conn) {
			if(conn != null) self._setConnections(conn.connectionSet); else self._setConnections(ui.AppContext.get_alias().get_connectionSet());
...
var defineWidget = function() {
	return { options : { label : null, isDragByHelper : true, containment : false, dndEnabled : true, classes : null, dropTargetClass : "labelDT", dragstop : null, cloneFcn : function(filterableComp,isDragByHelper,containment,dragstop) {
		...
		selfElement.addClass("label labelComp ").attr("id",StringTools.htmlEscape(self.options.label.name) + "_" + m3.util.UidGenerator.create(8));
		...
		var labelText = new $("<div>" + self.options.label.name + "</div>");
		...
		if(self.options.dndEnabled) {
			...
			if(!self.options.isDragByHelper) helper = "original"; else if(self.options.helperFcn != null && Reflect.isFunction(self.options.helperFcn)) helper = self.options.helperFcn;
			selfElement.on("dragstop",function(dragstopEvt,dragstopUi) {
				ui.AppContext.LOGGER.debug("dragstop on label | " + self.options.label.name);
				if(self.options.dragstop != null) self.options.dragstop(dragstopEvt,dragstopUi);
			});
	...
	return { _getDragStop : function() {
		...
		tags.droppable({ accept : function(d) {
			...
			if(isDuplicate(".connectionAvatar",_ui.draggable,tags,function(ele) {
				...
				return ui.widget.LabelCompHelper.getLabel(new $(ele)).get_iid();
			})) {
...
var defineWidget = function() {
	return { _createWidgets : function(selfElement,self) {
		...
		var connection = m3.helper.OSetHelper.getElementComplex(ui.AppContext.get_alias().get_connectionSet(),content.creator);
		...
		while(connIter.hasNext()) {
			...
			var connWithProfile = m3.helper.OSetHelper.getElement(ui.AppContext.get_alias().get_connectionSet(),ui.model.Connection.identifier(connection1));
		...
		ui.model.EM.addListener(ui.model.EMEvent.EditContentClosed,new ui.model.EMListener(function(content) {
			if(content.get_iid() == self1.options.content.get_iid()) selfElement1.show();
		}));
...
var defineWidget = function() {
	return { _create : function() {
		...
		var connFromAlias = m3.helper.OSetHelper.getElement(ui.AppContext.get_alias().get_connectionSet(),ui.model.Connection.identifier(conn));
...
var defineWidget = function() {
	return { _showNewLabelPopup : function(reference) {
		...
		popup = popup.popup({ createFcn : function(el) {
			...
			var parent = new $("<select id='labelParent' class='ui-corner-left ui-widget-content' style='width: 191px;'><option value='" + ui.AppContext.get_alias().rootLabelIid + "'>No Parent</option></select>").appendTo(container);
			...
			while(iter.hasNext()) {
				var label = iter.next();
				if(label.get_iid() != ui.AppContext.get_alias().rootLabelIid) {
					var option = "<option value='" + label.get_iid() + "'";
					if(self.selectedLabelComp != null && ui.widget.LabelCompHelper.getLabel(self.selectedLabelComp).get_iid() == label.get_iid()) option += " SELECTED";
			...
			createLabel = function() {
				if(input.val().length > 0) {
					...
					var eventData = new ui.model.CreateLabelData(label,parent.val());
		...
		ui.model.EM.addListener(ui.model.EMEvent.LabelCreated,new ui.model.EMListener(function(data) {
			self1._setLabels(ui.AppContext.get_alias().get_labelSet());
		},"LabelsList-LC"));
		ui.model.EM.addListener(ui.model.EMEvent.LabelUpdated,new ui.model.EMListener(function(label) {
			self1._setLabels(ui.AppContext.get_alias().get_labelSet());
		},"LabelsList-LU"));
		ui.model.EM.addListener(ui.model.EMEvent.LabelDeleted,new ui.model.EMListener(function(label) {
			self1._setLabels(ui.AppContext.get_alias().get_labelSet());
		},"LabelsList-LD"));
...
var defineWidget = function() {
	return { _create : function() {
		...
		var self = this;
		var selfElement1 = this.element;
		var alias = ui.AppContext.get_alias().name;
	...
	return { _create : function() {
		...
		if(user != null) {
			if(m3.helper.StringHelper.isNotBlank((function($this) {
				var $r;
				try {
					$r = user.get_currentAlias().data.imgSrc;
				} catch( __e ) {
					$r = "";
				}
				return $r;
			}(this)))) imgSrc = user.get_currentAlias().data.imgSrc; else if(m3.helper.StringHelper.isNotBlank(user.userData.imgSrc)) imgSrc = user.userData.imgSrc;
		}
