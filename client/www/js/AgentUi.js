...
m3.comm.BaseRequest.prototype = {
	start: function(opts) {
		...
		var ajaxOpts = { dataType : "json", contentType : "application/json", data : this.requestData, type : "POST", success : function(data,textStatus,jqXHR) {
			...
			if(jqXHR1.status == 403 && _g.onAccessDenied != null) return _g.onAccessDenied(jqXHR1,textStatus1,errorThrown);
			var error_message = "";
			if(errorThrown == null || typeof(errorThrown) == "string") {
				error_message = errorThrown;
				if(jqXHR1.message != null) error_message = jqXHR1.message; else if(jqXHR1.responseText != null && jqXHR1.responseText.charAt(0) != "<") error_message = jqXHR1.responseText;
			} else error_message = textStatus1 + ":  " + Std.string(errorThrown.message);
			if(_g.onError != null) _g.onError(jqXHR1,textStatus1,errorThrown); else m3.util.JqueryUtil.alert("There was an error making your request:  " + error_message);
		}};
...
m3.comm.LongPollingRequest = function(channel,requestToRepeat,logga,successFcn,ajaxOpts) {
	...
	var wrappedSuccessFcn = function(data,textStatus1,jqXHR1) {
		if(_g.running) try {
			successFcn(data,textStatus1,jqXHR1);
		} catch( e ) {
	...
	var errorFcn = function(jqXHR2,textStatus2,errorThrown) {
		_g.delayNextPoll = true;
		_g.logger.error("Error executing ajax call | Response Code: " + jqXHR2.status + " | " + jqXHR2.message);
	};
...
qoid.api.GetProfileMessage = function(connectionIids) {
	if(connectionIids == null) this.connectionIids = new Array(); else this.connectionIids = connectionIids;
};
...
qoid.api.Synchronizer.prototype = {
	dataReceived: function(c,dataObj) {
		...
		case "content":
			var _g11 = 0;
			var _g21;
			_g21 = js.Boot.__cast(data , Array);
			while(_g11 < _g21.length) {
				var content_ = _g21[_g11];
				++_g11;
		...
		case "introduction":
			var _g12 = 0;
			var _g22;
			_g22 = js.Boot.__cast(data , Array);
			while(_g12 < _g22.length) {
				var content_1 = _g22[_g12];
				++_g12;
				this.parms.introductions.push(qoid.AppContext.SERIALIZER.fromJsonX(content_1,qoid.model.Introduction));
			}
			break;
		case "label":
			var _g13 = 0;
			var _g23;
			_g23 = js.Boot.__cast(data , Array);
			while(_g13 < _g23.length) {
				var label_ = _g23[_g13];
				++_g13;
		...
		case "labelacl":
			var _g14 = 0;
			var _g24;
			_g24 = js.Boot.__cast(data , Array);
			while(_g14 < _g24.length) {
				var label_1 = _g24[_g14];
				++_g14;
				this.parms.labelAcls.push(qoid.AppContext.SERIALIZER.fromJsonX(label_1,qoid.model.LabelAcl));
			}
			break;
		case "labelChild":
			var _g15 = 0;
			var _g25;
			_g25 = js.Boot.__cast(data , Array);
			while(_g15 < _g25.length) {
				var labelChild_ = _g25[_g15];
				++_g15;
		...
		case "labeledContent":
			var _g16 = 0;
			var _g26;
			_g26 = js.Boot.__cast(data , Array);
			while(_g16 < _g26.length) {
				var labeledContent_ = _g26[_g16];
				++_g16;
		...
		case "notification":
			var _g17 = 0;
			var _g27;
			_g27 = js.Boot.__cast(data , Array);
			while(_g17 < _g27.length) {
				var content_2 = _g27[_g17];
				++_g17;
				this.parms.notifications.push(qoid.AppContext.SERIALIZER.fromJsonX(content_2,qoid.model.Notification));
			}
			break;
		case "profile":
			var _g18 = 0;
			var _g28;
			_g28 = js.Boot.__cast(data , Array);
			while(_g18 < _g28.length) {
				var profile_ = _g28[_g18];
				++_g18;
...
qoid.model.ContentType.__empty_constructs__ = [qoid.model.ContentType.AUDIO,qoid.model.ContentType.IMAGE,qoid.model.ContentType.URL,qoid.model.ContentType.TEXT];
...
qoid.model.VerificationRequestData.prototype = {
	getContent: function() {
		var fromJson = { iid : this.contentIid, deleted : false, contentType : Std.string(this.contentType), data : this.contentData};
...
var defineWidget = function() {
	return { _createWidgets : function(selfElement,self) {
		...
		if((function($this) {
			...
			$r = this1.get(self.options.content.aliasIid);
			return $r;
		}(this)) != null) aliasIid = self.options.content.aliasIid; else if((function($this) {
			var $r;
			var this2 = qoid.AppContext.CONNECTIONS.delegate();
			$r = this2.get(self.options.content.connectionIid);
			return $r;
		}(this)) != null) connectionIid = self.options.content.connectionIid;
		...
		if((function($this) {
			...
			$r = this3.get(self.options.content.iid);
			return $r;
		}(this)) == null) qoid.AppContext.GROUPED_LABELEDCONTENT.addEmptyGroup(self.options.content.iid);
		self.onchangeLabelChildren = function(ele,evt1) {
			if(evt1.isAdd()) {
				if(ele["is"](".connectionAvatar")) postConnections.append(ele); else postLabels.append(ele);
			} else if(evt1.isUpdate()) throw new m3.exception.Exception("this should never happen"); else if(evt1.isDelete()) ele.remove();
		};
		self.mappedLabels = new m3.observable.MappedSet((function($this) {
			var $r;
			var this4 = qoid.AppContext.GROUPED_LABELEDCONTENT.delegate();
			$r = this4.get(self.options.content.iid);
			return $r;
		}(this)),function(lc) {
			var connection = qoid.AppContext.connectionFromMetaLabel(lc.labelIid);
		...
		var selfElement2 = this.element;
		if(self2.menu == null) {
			var menu1 = new $("<ul id='contentCompMenu-" + self2.options.content.iid + "'></ul>");
			menu1.appendTo(selfElement2);
			var menuOptions = [];
			var menuOption;
			menuOption = { label : "Edit...", icon : "ui-icon-pencil", action : function(evt3,m) {
				evt3.stopPropagation();
			...
			menuOptions.push(menuOption);
			menuOption = { label : "Delete...", icon : "ui-icon-circle-close", action : function(evt4,m1) {
			...
			menuOptions.push(menuOption);
			menuOption = { label : "Request Verification...", icon : "ui-icon-circle-triangle-n", action : function(evt5,m2) {
				evt5.preventDefault();
...
var defineWidget = function() {
	return { _create : function() {
		...
		var $it3 = qoid.AppContext.CONNECTIONS.iterator();
		while( $it3.hasNext() ) {
			var conn = $it3.next();
			var div = new $("<div></div>");
