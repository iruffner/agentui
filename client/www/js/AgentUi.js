...
agentui.api.EventDelegate.init = function() {
	agentui.model.EM.addListener("FILTER_RUN",function(filterData) {
		qoid.QoidAPI.query(new qoid.RequestContext("filteredContent",m3.util.UidGenerator.create(12)),"content",filterData.filter.q,true,true,filterData.connectionIids);
	});
	agentui.model.EM.addListener("UseAlias",function(alias) {
		qoid.QoidAPI.useAlias(alias);
	});
	agentui.model.EM.addListener("DeleteAlias",function(alias1) {
		qoid.QoidAPI.deleteAlias(alias1.iid);
	});
	agentui.model.EM.addListener("UpdateAlias",function(alias2) {
	...
	agentui.model.EM.addListener("CreateLabel",function(data2) {
		qoid.QoidAPI.createLabel(data2.parentIid,data2.label.name,data2.label.data);
	});
	agentui.model.EM.addListener("UpdateLabel",function(data3) {
		qoid.QoidAPI.updateLabel(data3.label.iid,data3.label.name,data3.label.data);
	});
	agentui.model.EM.addListener("MoveLabel",function(data4) {
		qoid.QoidAPI.moveLabel(data4.label.iid,data4.parentIid,data4.newParentId);
	});
	agentui.model.EM.addListener("CopyLabel",function(data5) {
		qoid.QoidAPI.copyLabel(data5.label.iid,data5.newParentId);
	});
	agentui.model.EM.addListener("DeleteLabel",function(data6) {
		qoid.QoidAPI.deleteLabel(data6.label.iid,data6.parentIid);
	});
	agentui.model.EM.addListener("RespondToIntroduction",function(intro) {
		if(intro.accepted) qoid.QoidAPI.acceptIntroduction(intro.notificationIid); else qoid.QoidAPI.consumeNotification(intro.notificationIid);
	});
	agentui.model.EM.addListener("GrantAccess",function(parms) {
		qoid.QoidAPI.grantAccess(parms.labelIid,parms.connectionIid,1);
	});
	agentui.model.EM.addListener("RevokeAccess",function(parms1) {
	...
	agentui.model.EM.addListener("AcceptVerification",function(v) {
		qoid.QoidAPI.acceptVerification(v);
	});
	agentui.model.EM.addListener("RejectVerificationRequest",function(notificationIid) {
		qoid.QoidAPI.rejectVerificationRequest(notificationIid);
	});
	agentui.model.EM.addListener("RejectVerification",function(notificationIid1) {
		qoid.QoidAPI.rejectVerificationResponse(notificationIid1);
	});
};
agentui.model = {};
...
agentui.model.ContentSource.removeListener = function(id) {
	var i = m3.helper.ArrayHelper.indexOfComplex(agentui.model.ContentSource.listeners,id,"id");
	if(i > -1) {
		agentui.model.ContentSource.listeners[i].destroy();
		agentui.model.ContentSource.listeners.splice(i,1);
	}
};
...
agentui.model.ContentSource.onLoadFilteredContent = function(data) {
	if(data.result.standing || agentui.model.ContentSource.context != null && agentui.model.ContentSource.context.handle == data.context.handle) agentui.model.ContentSource.addContent(data.result.results,data.result.route[0]); else {
		agentui.model.ContentSource.clearQuery();
		if(typeof(data.context) == "string") agentui.model.ContentSource.context = m3.serialization.Serializer.get_instance().fromJsonX(JSON.parse(data.context),qoid.RequestContext); else agentui.model.ContentSource.context = m3.serialization.Serializer.get_instance().fromJsonX(data.context,qoid.RequestContext);
		agentui.model.ContentSource.beforeSetContent();
		agentui.model.ContentSource.addContent(data.result.results,data.result.route[0]);
	}
};
agentui.model.ContentSource.clearQuery = function() {
	if(agentui.model.ContentSource.context != null) {
		m3.log.Logga.get_DEFAULT().warn("deregisterSqueries");
		qoid.QoidAPI.cancelQuery(new qoid.RequestContext(agentui.model.ContentSource.context.context,agentui.model.ContentSource.context.handle));
		agentui.model.ContentSource.filteredContent.clear();
		agentui.model.ContentSource.context = null;
	}
};
...
var defineWidget = function() {
	return { _create : function() {
		...
		while( $it0.hasNext() ) {
			var alias3 = $it0.next();
			var alias4 = [alias3];
			var icon = "ui-icon-person";
			menuOption = { label : alias4[0].profile.name, icon : icon, action : (function(alias4) {
				return function(evt1,m) {
					if(qoid.model.Alias.identifier(qoid.Qoid.get_currentAlias()) == qoid.model.Alias.identifier(alias4[0])) menu.hide(); else {
						...
						agentui.model.EM.change("UseAlias",alias4[0]);
	...
	return { initialized : false, _create : function() {
		...
		var updateBtn = new $("<button>" + (alias2 != null?"Update":"Create") + "</button>").appendTo(btnDiv1).button().click(function(evt8) {
			...
			if(alias2 == null) {
				...
				applyDlg = function() {
					agentui.model.EM.listenOnce("onAddAlias",function(alias3) {
						haxe.Timer.delay(function() {
				...
				alias2.profile.imgSrc = profilePic;
				applyDlg = function() {
					agentui.model.EM.listenOnce("onUpdateAlias",function(alias4) {
						self3._showAliasDetail(alias4);
					});
					qoid.QoidAPI.updateAliasProfile(alias2.iid,name,profilePic);
				};
			}
			applyDlg();
		});
	...
	return { options : { connection : null, classes : null}, _create : function() {
		...
		(js.Boot.__cast(selfElement , $)).droppable({ accept : function(d) {
			...
			if(!dropper.equals(droppee)) {
				var intro = null;
				if(intro != null) {
					if(intro.bAccepted && intro.aAccepted) m3.util.JqueryUtil.alert("These two aliases are already connected");
		...
		self.notifications = new m3.observable.FilteredSet(qoid.Qoid.notifications,function(n) {
			return n.get_connectionIid() == self.options.connection.iid;
		});
		self.notificationsListener = function(i,evt2) {
		...
		if(self5.notifications != null) self5.notifications.removeListener(self5.notificationsListener);
	}};
};
...
var defineWidget = function() {
	return { _create : function() {
		...
		menu.m3menu({ wrapLabelInAtag : true, classes : "container shadow", menuOptions : [{ label : "Revoke Access...", icon : "ui-icon-circle-plus", action : function(evt,m) {
		...
		self._mapListener = function(conn,connComp,evt3) {
			if(evt3.isAdd()) {
				...
				concomps.each(function(i,dom) {
					if(!found) {
						var comp = new $(dom);
						if(agentui.widget.ConnectionCompHelper.connection(comp).data.name > conn.data.name) {
							connComp.insertBefore(comp);
							found = true;
						}
					}
				});
				if(!found) spacer.before(connComp);
			} else if(evt3.isUpdate()) agentui.widget.ConnectionCompHelper.update(connComp,conn); else if(evt3.isDelete()) connComp.remove();
...
var defineWidget = function() {
	return { _showVerifications : function(ref) {
		...
		if(self3.menu == null) {
			...
			menuOptions.push(menuOption);
			menu1.m3menu({ menuOptions : menuOptions, wrapLabelInAtag : true}).hide();
			self3.menu = menu1;
		}
		return self3.menu;
...
var defineWidget = function() {
	return { _create : function() {
		...
		if(filterables1.length == 0) {
			if(agentui.widget.LiveBuildToggleHelper.isLive(liveToggle1)) agentui.model.ContentSource.clearQuery();
		} else {
...
var defineWidget = function() {
	return { _create : function() {
		...
		var profile = m3.helper.OSetHelper.getElementComplex(qoid.Qoid.profiles,self.options.notification.props.connectionIid,function(p) {
		...
		if(profile != null) {
			...
			imgDiv.empty().append(new $("<img class='intro-profile-img container' src='" + imgSrc + "'/>"));
		}
		...
		var listener1 = null;
		var listener2 = null;
		listener1 = agentui.model.EM.listenOnce("onConsumeNotification",function(e) {
			self1.destroy();
			selfElement1.remove();
			agentui.model.EM.removeListener("onAcceptIntroduction",listener2);
		});
		listener2 = agentui.model.EM.listenOnce("onAcceptIntroduction",function(e1) {
			self1.destroy();
			selfElement1.remove();
			agentui.model.EM.removeListener("onConsumeNotification",listener1);
		});
		var confirmation = new qoid.model.IntroResponseMessage(self1.options.notification.iid,accepted);
...
var defineWidget = function() {
	return { getSelected : function() {
		...
		menu.m3menu({ wrapLabelInAtag : true, classes : "container shadow", menuOptions : [{ label : "New Child Label", icon : "ui-icon-circle-plus", action : function(evt5,m) {
			evt5.stopPropagation();
...
var defineWidget = function() {
	return { _create : function() {
		...
		agentui.model.EM.addListener(qoid.QE.onInitiateIntroduction,function(n) {
			selfElement1.dialog("close");
		},"RequestIntroductionDialog-Introduction-Response");
		qoid.QoidAPI.initiateIntroduction(intro.aConnectionIid,intro.aMessage,intro.bConnectionIid,intro.bMessage);
	}, _buildDialog : function() {
		var self2 = this;
		var selfElement2 = this.element;
...
var defineWidget = function() {
	return { _create : function() {
	...
	}, _createConnectionSelection : function(connection,connectionContainer1) {
		var connComp = new $("<div class='connection-selection' connection_name='" + connection.data.name + "'></div>");
		connComp.append("<input type='checkbox' class='conn_cb' id='cb_" + connection.iid + "'/>");
		var avatar = new $("<div class='avatar'></div>").connectionAvatar({ connectionIid : connection.iid, dndEnabled : false, isDragByHelper : true, containment : false}).appendTo(connComp).css("display","inline");
		connComp.append("<div class='labelDiv' style='display:inline'>" + connection.data.name + "</div>");
		var found = false;
		var concomps = connectionContainer1.find(".connection-selection");
		concomps.each(function(i,dom) {
			if(!found) {
				var comp = new $(dom);
		...
		if(!found) connectionContainer1.append(connComp);
	}, initialized : false, _sendRequest : function() {
		var self1 = this;
		var selfElement1 = this.element;
		var connectionIids = new Array();
		new $(".conn_cb").each(function(i1,dom1) {
			var cb = new $(dom1);
			if(cb.prop("checked")) connectionIids.push(cb.attr("id").split("_")[1]);
		});
		var vr = new qoid.model.VerificationRequest(self1.options.content,connectionIids,new $("#vr_message").val());
		agentui.model.EM.listenOnce("onVerificationRequest",function(n) {
			selfElement1.dialog("close");
		});
...
agentui.model.EMEvent.UseAlias = "UseAlias";
...
agentui.model.EMEvent.OnConsumeNotification = "onConsumeNotification";
...
m3.observable.SortedSet.__rtti = "<class path=\"m3.observable.SortedSet\" params=\"T\" module=\"m3.observable.OSet\">\n\t<extends path=\"m3.observable.AbstractSet\"><c path=\"m3.observable.SortedSet.T\"/></extends>\n\t<_source><c path=\"m3.observable.OSet\"><c path=\"m3.observable.SortedSet.T\"/></c></_source>\n\t<_sortByFn><f a=\"\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<c path=\"String\"/>\n</f></_sortByFn>\n\t<_sorted><c path=\"Array\"><c path=\"m3.observable.SortedSet.T\"/></c></_sorted>\n\t<_dirty><x path=\"Bool\"/></_dirty>\n\t<_comparisonFn><f a=\":\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<x path=\"Int\"/>\n</f></_comparisonFn>\n\t<sorted public=\"1\" set=\"method\" line=\"576\"><f a=\"\"><c path=\"Array\"><c path=\"m3.observable.SortedSet.T\"/></c></f></sorted>\n\t<indexOf set=\"method\" line=\"584\"><f a=\"t\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<x path=\"Int\"/>\n</f></indexOf>\n\t<binarySearch set=\"method\" line=\"589\"><f a=\"value:sortBy:startIndex:endIndex\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<c path=\"String\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n\t<x path=\"Int\"/>\n</f></binarySearch>\n\t<delete set=\"method\" line=\"607\"><f a=\"t\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<x path=\"Void\"/>\n</f></delete>\n\t<add set=\"method\" line=\"611\"><f a=\"t\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<x path=\"Void\"/>\n</f></add>\n\t<identifier public=\"1\" set=\"method\" line=\"617\" override=\"1\"><f a=\"\"><f a=\"\">\n\t<c path=\"m3.observable.SortedSet.T\"/>\n\t<c path=\"String\"/>\n</f></f></identifier>\n\t<iterator public=\"1\" set=\"method\" line=\"621\" override=\"1\"><f a=\"\"><t path=\"Iterator\"><c path=\"m3.observable.SortedSet.T\"/></t></f></iterator>\n\t<delegate public=\"1\" set=\"method\" line=\"625\" override=\"1\"><f a=\"\"><x path=\"Map\">\n\t<c path=\"String\"/>\n\t<c path=\"m3.observable.SortedSet.T\"/>\n</x></f></delegate>\n\t<new public=\"1\" set=\"method\" line=\"536\"><f a=\"source:?sortByFn\" v=\":null\">\n\t<c path=\"m3.observable.OSet\"><c path=\"m3.observable.SortedSet.T\"/></c>\n\t<f a=\"\">\n\t\t<c path=\"m3.observable.SortedSet.T\"/>\n\t\t<c path=\"String\"/>\n\t</f>\n\t<x path=\"Void\"/>\n</f></new>\n</class>";
